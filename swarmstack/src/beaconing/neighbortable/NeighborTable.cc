...
NeighborTableEntry::NeighborTableEntry (MacAddress id, simtime_t lastB, Coord lastPos, Coord lastVel, double alpha, double time)
  :nbId(id),
   lastBeacon(lastB),
...
void NeighborTable::recordBeacon (const Beacon* beacon)
{
  ...
  if (entryAvailable(id))
    ...
    {
      NeighborTableEntry newEntry = NeighborTableEntry(id, rxTime, pos, vel, ewmaAlpha, -1);
      nbTable[id] = newEntry;
    }
}
